...
// Function to handle pulse interrupts for a specific meter
void pulse_callback_for_meter(const Meter& meter) {
    int gpio = meter.pin;
...
// Function to handle pulse interrupts for all meters
void pulse_callback_all_meters() {
    for (const auto& meter : meters) {
        pulse_callback_for_meter(meter.second);
    }
}

// FreeRTOS task for checking each energy meter pin every 2 seconds
void meterCheckTask(void *parameter) {
    while (1) {
        pulse_callback_all_meters();
...
void setup() {
    ...
    setPins();
       ...
       // Create FreeRTOS task for checking each energy meter pin every 2 seconds
    xTaskCreate(meterCheckTask, "MeterCheckTask", 4096, NULL, 1, NULL);
}
